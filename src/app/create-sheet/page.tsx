...
import { Printer, Download, RotateCw, ZoomIn, ZoomOut, ChevronRight, Grid2x2, Image as ImageIcon, CreditCard, FileText, Scissors, Coins, Settings, User } from 'lucide-react'
    ...
    <DashboardLayout>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8 }}>
          <div style={{ width: 46, height: 46, borderRadius: 12, background: 'rgba(59,130,246,0.12)', color: '#3b82f6', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Grid2x2 size={22} />
          </div>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 800, color: 'var(--text-primary)', margin: '0 0 4px 0', letterSpacing: '-0.4px' }}>Create Sheet</h1>
            <p style={{ fontSize: 13.5, color: 'var(--text-muted)', margin: 0 }}>Generate passport-sized A4 photo sheets with custom layout, background colors, and print-ready PDF export.</p>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 24, padding: '12px 0', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', marginTop: 16 }}>
          {[
            { label: 'Photos / Page', value: '42', color: '#3b82f6' },
            { label: 'Grid Layout', value: '6 × 7', color: '#10b981' },
            { label: 'Photo Size', value: '3.5×4.5cm', color: '#8b5cf6' },
            { label: 'Output', value: 'A4 PDF', color: '#f59e0b' },
          ].map(s => (
            <div key={s.label}>
          ...
          {totalPages > 1 && (
            <div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 2 }}>Pages</div>
              <div style={{ fontSize: 15, fontWeight: 800, color: '#ec4899' }}>{totalPages}</div>
            </div>
          )}
        </div>
      </div>
